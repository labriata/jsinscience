...
function analyzeabstracts(whattosearch,wheretoput)
{    
  var whattosearch = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&retmax=1000&term=\"' + whattosearch + '\"'
...
function iterateJSON(idlist, publications, wheretoput) {
    ...
    $.getJSON('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id='+id+'&retmode=json', function(summary){
        ...
        citation+='<b>' + summary.result[id].title+' <i>'+summary.result[id].fulljournalname+'</i></b> ' + summary.result[id].pubdate + '   [<a href=\"https://www.ncbi.nlm.nih.gov/pubmed/' + id + '\" target=\"_blank\">See in PubMed</a>]<p>';
        ...
        if(idlist.length!=0){
        	...
        	nn=nn-1
            $("#"+wheretoput).append("<p>" + nn + " DONE")
        }
    });
}


function analyzeabstract(id){
  $.get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&retmode=text&rettype=abstract&id=" + id, function(abstracttext){
     tmp = abstracttext.toLowerCase().split("\n")
     tmp = tmp.join(' ')
     if (tmp.match('mutant') || tmp.match('mutat'))
     {
        $("#mutantresults").append('<hr>')
        var re = new RegExp('( (A|C|D|E|F|G|H|I|K|L|M|N|P|Q|R|S|T|V|W|Y)\[0-9]+)|((ala|cys|asp|glu|phe|gly|his|ile|lys|leu|met|asn|pro|gln|arg|ser|thr|val|trp|tyr)\[0-9]+)|((ala|cys|asp|glu|phe|gly|his|ile|lys|leu|met|asn|pro|gln|arg|ser|thr|val|trp|tyr) \[0-9]+)|((alanine|cysteine|aspartate|glutamate|phenylalanine|glycine|histidine|isoleucine|lysine|leucine|methionine|asparagine|proline|glutamine|arginine|serine|threonine|valine|tryptophane|tyrosine) \[0-9]+)','gi');
        var results = new Array();//this is the results you want
        while (re.exec(tmp)){
          $("#mutantresults").append('<p>...' + tmp.substr(re.lastIndex-50, 100) + '... [<a href=\"https://www.ncbi.nlm.nih.gov/pubmed/' + id + '\" target=\"_blank\">See in PubMed</a>]')
        }
     }
  });
}
...
function analyzeabstract2(id,title){
  $.get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&retmode=text&rettype=abstract&id=" + id, function(abstracttext){
